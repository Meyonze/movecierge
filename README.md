...
- 全国1,740市区町村を検索対象にしており、1,569件の窓口情報を収録しています。未収録の自治体は全国共通ルールと公式サイト検索へのリンクで案内します。
- 収録データには自動調査の結果も含まれます。窓口情報のみを先行掲載する低信頼度の候補は、画面上で要確認と明示し、公式検索へ案内します。
- 完了にしたチェック項目は、その端末のブラウザ内に保存されます。住所ごとの手続きと全国共通の手続きを分けて保持し、外部には送信しません。
- 引っ越し予定日を入れると、転出・転入の目安を日付つきで表示します。生成したチェックリストは、共有用リンク・コピー・印刷に対応しており、家族との確認や引っ越し当日の持ち出し用に使えます。
...
スクリプトは `data/cities.json` の自治体IDを正として、画面が読むJSON形式のまま保存します。窓口・転出・転入の情報がそろった結果は `confidence: high` として `data/city_data.json` に入れます。窓口情報のみ確認できたものは `confidence: low` として要確認表示付きで公開し、それ以外は `data/city_data_review.json` に分離します。レビュー済みの候補を再調査する場合は `--retry-reviewed` を付けてください。
